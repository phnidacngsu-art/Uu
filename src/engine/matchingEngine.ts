...
function extractColor(
  ...
  if ('colorName' in item && 'hexCode' in item) {
    return { name: item.colorName, hex: item.hexCode };
  }
  if ('name' in item && 'hex' in item) {
...
// Safe Neutral palettes that can be safely used unless they clash with Kali
const SAFE_NEUTRALS = [
...
export function runMatchingEngine(
  ...
  const kaliHexes = new Set(kaliRules.map((k) => k.hexCode?.toUpperCase()).filter(Boolean));
  const avoidHexes = new Set(avoidRules.map((a) => a.hexCode?.toUpperCase()).filter(Boolean));
  ...
  const defaultNeutral = cleanNeutrals[0] || { name: 'ขาวออฟไวท์', hex: '#F8FAFC' };
  ...
  // If user has a preferred color and it's not Kali, prioritize it
  if (preferredColors && preferredColors.length > 0) {
    const validPreferred = preferredColors.filter((c) => !isKali(c));
    if (validPreferred.length > 0) {
      cleanRecommended.unshift({
        id: 'pref-color-user',
        dayOfWeek: targetDayKey,
        colorName: 'สีที่คุณชื่นชอบ',
        hexCode: validPreferred[0],
        type: 'recommended',
        priority: 0,
        description: 'สีที่คุณต้องการเน้นเป็นพิเศษในวันนี้',
        isActive: true,
        createdAt: '',
        updatedAt: '',
      });
  ...
  // Harmony Strategy 1: Monochromatic / Tonal Harmony
  {
    ...
    const secondary = cleanSecondary[0] || cleanNeutrals[0] || defaultNeutral;
    const neutral = cleanNeutrals[0] || defaultNeutral;
    const accent = cleanSecondary[1] || cleanRecommended[1] || cleanNeutrals[5] || defaultNeutral;
  ...
  // Harmony Strategy 2: Analogous / Complementary Contrast
  {
    ...
    const neutral = cleanNeutrals[1] || cleanNeutrals[0] || defaultNeutral;
  ...
  // Harmony Strategy 3: Relaxed Sophisticated Minimal
  {
    ...
    const neutral1 = cleanNeutrals[0] || defaultNeutral;
    const neutral2 = cleanNeutrals[2] || cleanNeutrals[1] || cleanNeutrals[0] || defaultNeutral;
  ...
  return {
    ...
    notice:
      kaliRules.length > 0
        ? `ระบบคัดแยกสีอวมงคล (กาลกิณี) ออก ${kaliRules.length} สีโดยอัตโนมัติ เพื่อเสริมความเป็นสิริมงคลสูงสุด`
        : 'สีมงคลคัดสรรตามหลักทักษาปกรณ์และทฤษฎีคู่สีแฟชั่นสากล',
  };
}
